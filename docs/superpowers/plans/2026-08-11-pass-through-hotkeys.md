...
> **철회됨 (2026-08-13).** 이 계획으로 만든 Raw Input 경로는 실기 검증에서 목표를 달성하지 못했고 기존에 동작하던 조합(`Ctrl+1` 등)을 잃어 `src/`와 `tests/`에서 제거했다. 근거는 `docs/superpowers/specs/2026-08-13-hotkey-detection-findings.md`를 참고한다. 아래 내용은 기록으로만 남긴다.
...
실행 기록 (2026-08-11, 사용자 보고): **부분 PASS / 전체 단계는 미완료.** 사용자가 메모장/브라우저에서 (a) `F2`가 활성 앱에 전달되면서 타이머 1도 실행됨, (b) 정확한 `Ctrl+F5`만 타이머 2를 실행하고 `F5`/`Ctrl+Shift+F5`는 실행하지 않음, (c) `F2` 길게 누름은 한 번만 실행되고 키를 놓은 뒤 재무장되며 트레이 숨김 상태도 동작함, (d) 단축키 캡처 중 타이머 실행이 억제되고 캡처 종료 뒤 재개됨을 PASS로 보고했고 실패는 보고하지 않았다. 한국어·영어 배열에서 `A`와 OEM 기호 키의 WPF 표시와 Raw Input 가상 키 일치 검사는 **NOT RUN / requires user**로 남는다. OS 예약 조합은 지원 대상이 아니다.
...
실행 기록 (2026-08-12, 게임 없는 대체 실험): 사용자가 테일즈위버 활성 시 `F1`~`F12`가 게임에서는 동작하지만 타이머는 반응하지 않는다고 보고했다(두 프로그램 모두 일반 권한). 게임을 실행할 수 없는 환경이라 `tools/RawInputProbe`(수신 계측)와 `tools/ForegroundAntagonist`(가로채기 재현)로 후보 메커니즘을 이 PC에서 시험했다. 판정은 사람의 청각이 아니라 대조키 `Q`와 프로브 로그로 했다. 결과: (1) 포그라운드 앱이 `RIDEV_NOLEGACY | RIDEV_NOHOTKEYS`로 키보드를 독점해도 현재 구현은 `F5`를 정상 수신한다 → 원인 후보에서 제외. (2) 포그라운드 앱이 저수준 키보드 훅으로 `F1`~`F12`를 삼키면 `F5` 패킷이 0개가 된다(같은 구간에서 대조키 `Q`는 수신, 하트비트 11개로 프로세스는 생존) → **Raw Input으로 관찰 불가**. (3) `GetAsyncKeyState` 폴링도 같은 조건에서 `F5` 0개, `Q` 1개 → **사용자 모드에 남는 관찰 경로 없음**. (4) `RIDEV_EXINPUTSINK`는 수신 범위를 넓히는 플래그가 아니라 포그라운드 앱이 해당 입력을 소비하지 않을 때만 받는 **좁히는** 플래그였다. 상대역이 키보드 Raw Input을 등록하자 수신이 통째로 0(`kbd=0`)이 됐다. 따라서 이 플래그를 제품에 추가하면 회귀가 된다. 등록 플래그는 현재의 `RIDEV_INPUTSINK | RIDEV_DEVNOTIFY`를 유지한다. 상세 표와 로그 해석은 `tools/RawInputProbe/README.md`에 있다. **미해결로 남은 것:** 테일즈위버가 실제로 이 방식으로 `F1`~`F12`를 선점하는지, 그리고 게임이 쓰지 않는 키(`Insert`, `Pause` 등)는 통과하는지. 둘 다 게임 실행이 필요하므로 아래 항목은 계속 미완료로 둔다.

실행 기록 (2026-08-11): **NOT RUN / user deferred.** 현재 환경에서는 사용자가 테일즈위버 창 모드/전체 화면 모드와 BlackCipher·기타 보안 경고 검사를 수행할 수 없어 나중에 시험한다. 실제 대화형 사용자 세션에서 일반 사용자 권한으로 테일즈위버를 실행하고, 각 모드의 F2/F5 게임 동작과 대응 타이머 동시 실행, 길게 누름 시 게임 연속 동작과 타이머 1회 실행, 게임 비종료, 보안 경고 미표시, OS 예약 조합 비보장을 보고해야 한다. 이 작업에서는 입력 합성·후킹·주입, 게임/보안 프로세스 접근을 하지 않았다.
